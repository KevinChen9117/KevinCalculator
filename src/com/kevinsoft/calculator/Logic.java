...
import com.kevinsoft.calculator.CalculatorDisplay.Scroll;
...
class Logic {
    ...
    private void clear(boolean scroll) {
        mHistory.enter("", "");
        mDisplay.setText("0", scroll ? CalculatorDisplay.Scroll.UP : CalculatorDisplay.Scroll.NONE);
        cleared();
    }
    ...
    void onDelete() {
        if (getText().equals(mResult) || mIsError) {
            ...
            Log.d("TCL", "current :" + mDisplay.getEditText().getText().toString());
            if(TextUtils.isEmpty(mDisplay.getEditText().getText().toString())){
            	mDisplay.setText("0", CalculatorDisplay.Scroll.UP);
            }
            mResult = "";
        }
    }
